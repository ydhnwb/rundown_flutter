...
class FriendBloc extends Bloc<FriendEvent, FriendState>{
  ...
  Stream<FriendState> mapEventToState(FriendEvent event) async* {
    ...
    }else if(event is CheckFriendshipStatus){
      yield* _checkFriendshipStatus(event.userId);
    }
  }
  ...
  Stream<FriendState> _checkFriendshipStatus(String userId) async* {
    try{
      yield FriendLoadingState();
      _dio.options.headers["Authorization"] = "Token d3b61525fb85366e5f3780266494f4bb92cb4f57";
      Response response = await _dio.post(RestClient.CHECK_FRIENDSHIP_STATUS, data: {
        'id' : userId
      });
      ...
      if(baseResponse.status){
        ...
        yield FriendErrorState(message: "Cannot fetch user data.");
      }
    }catch(e){
  ...
  Stream<FriendState> _allFriend() async*{
    try{
      ...
      BaseResponse baseResponse = BaseResponse.fromJson(res.data);
      if(baseResponse.status){
        Map<String, dynamic> data = baseResponse.data;
        List<Friend> friends = List();
        List<Friend> requested = List();
        List<Friend> friendRequests = List();
        var f = data["friends"].toList();
        print(f);
        yield FriendLoadedState(friends: friends, requested: requested, friendRequests: friendRequests);
      }else{
        yield FriendErrorState(message: "Cannot accept friend request.");
      }
    }catch(e){
